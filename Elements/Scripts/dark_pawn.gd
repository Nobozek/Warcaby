...
var pawn = GameFunctions.Pawn.new(2, [Vector2(1, 1),Vector2(-1, 1)], self, 7)
var queen

func _ready() -> void:
	input_event.connect(pawn._on_input_event)
	GameData.red_pawns.append(pawn)
...
func become_queen():
	for i in range(GameData.red_pawns.size()):
		if GameData.red_pawns[i].get_grid_pos() == pawn.get_grid_pos():
			GameData.red_pawns.remove_at(i)
			break
	input_event.disconnect(pawn._on_input_event)
	pawn = null
	pawn = GameFunctions.Queen.new(2, self)
	GameData.red_pawns.append(pawn)
